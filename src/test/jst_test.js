...
function testParseTemplate() {
    Utl.assert("" == new JST().parse(), "test parse null template");
    Utl.assert("" == new JST().parse(""), "test parse empty template");
    Utl.assert("" == new JST().parse("<%-- comment --%>"), "test parse only-comment template");
...
function testRunTemplate() {
    ...
    Utl.assert("" == new JST().run(), "test run null template");
    Utl.assert("" == new JST().run(""), "test run empty template");
    Utl.assert("" == new JST().run("<%-- comment --%>"), "test run only-comment template");
    Utl.assert("foo" == new JST().run("foo"), "test run simple template");
    Utl.assert("\n\rthis is foo@bar.com's \"template\"!" == new JST().run("\n\rthis is foo@bar.com's \"template\"!"), "test run escaped-content template");
    Utl.assert("foo" == new JST().run("<% out(\"foo\"); %>"), "test run single-scriptlet template");
    Utl.assert("foobarbaz" == new JST().run("<% out(\"foo\"); %><%! out('bar'); %><%= 'baz' %><%@ baal %><%-- comment --%>"), "test run multiple-scriptlet template");
    Utl.assert("bar" == new JST().run("<%=foo%>", { foo: "bar" }), "test run single-argument template");
    Utl.assert("123" == new JST().run("<%=foo%><%=bar%><%=baz%>", { foo: 1, bar: 2, baz: 3 }), "test run multiple-argument template");
